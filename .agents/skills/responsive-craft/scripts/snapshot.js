...
const DEV_BROWSER_TMP = path.join(
  require("os").homedir(),
  ".dev-browser",
  "tmp"
);
...
function parseArgs(args) {
  ...
  return { breakpoints, isBefore, outputDir, url };
}
...
// Main
const { url, breakpoints, isBefore, outputDir } = parseArgs(
...
const validIndices = screenshots
  .map((s, i) => (s ? i : -1))
  .filter((i) => i >= 0);
...
if (!isBefore) {
  ...
  if (fs.existsSync(beforeDir)) {
    ...
    const matchingBps = breakpoints.filter(
      (bp, i) => beforeShots[i] && screenshots[i]
    );
    ...
    if (matchingBps.length > 0) {
      ...
      console.log(
        "No matching breakpoints between before and after — skipping comparison."
      );
    }
  }
}
